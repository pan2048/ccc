...
struct ASTnode *memberAccess(struct ASTnode *left, int withpointer)
{
  ...
  if (withpointer && left->type != typePointerTo(P_STRUCT) && left->type != typePointerTo(P_UNION))
  ...
  left = astMakeNode(A_ADD, typePointerTo(m->type), m->ctype, left, NULL, right, NULL, 0);
