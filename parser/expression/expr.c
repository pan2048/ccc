...
struct ASTnode *expression_list(int endtoken) {
  ...
  while (Token.token != endtoken) {
    ...
    tree =
      astMakeNode(A_GLUE, P_NONE, NULL, tree, NULL, child, NULL, exprcount);
...
static struct ASTnode *funccall(void) {
  ...
  tree =
    astMakeUnary(A_FUNCCALL, funcptr->type, funcptr->ctype, tree, funcptr, 0);
...
static struct ASTnode *array_access(struct ASTnode *left) {
  ...
  left =
    astMakeNode(A_ADD, left->type, left->ctype, left, NULL, right, NULL, 0);
  left =
    astMakeUnary(A_DEREF, value_at(left->type), left->ctype, left, NULL, 0);
  return (left);
}
...
static struct ASTnode *member_access(struct ASTnode *left, int withpointer) {
  ...
  left =
    astMakeNode(A_ADD, pointer_to(m->type), m->ctype, left, NULL, right, NULL,
	      0);
...
struct ASTnode *binexpr(int ptp) {
  ...
  while ((op_precedence(tokentype) > ptp) ||
    ...
    left =
      astMakeNode(binastop(tokentype), left->type, left->ctype, left, NULL,
		right, NULL, 0);
